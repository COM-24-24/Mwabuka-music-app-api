...
export class ReleaseService {
  ...
  async createRelease(dto: CreateReleaseDto): Promise<Release> {
    ...
    const release = await this.releaseRepository.save(newRelease);

    await this.notifyService.createAnnouncement({
      type: NotificationType.INFO,
      title: 'New release available',
      message: `${release.title} is now available.`,
    });
  ...
  async updateRelease(
    ...
    const updateData: Partial<CreateReleaseDto> = { ...dto };
    if (dto.releaseDate) {
